...
Variable    = "elPt"
MCToys      = ReadToysMC(     MC_B,   channel, Variable, indice[kindice])
DataToys    = ReadToysMC(     Data_B, channel, Variable, indice[kindice])
MCTemplates = ReadMCTemplate( MC,     channel, Variable, indice[kindice])
MCNominal   = ReadMCFiles(    MC,     channel, Variable, indice[kindice])
DataNominal = ReadMCFiles(    Data,   channel, Variable, indice[kindice])
...
for i in range(0, len(MCToys)):
    ...
    DataToysO.append( DataToys[i].Clone() )

for toy in range(0, len(MCToys)):

	# Add Nominal distribution to Toys:
	for i in range(0, DataNominal.GetNbinsX()):
	...
	# add nominal distribution to bootstrap variation to creat Templates:
	for i in range(0, len(MCTemplates)):
	    for j in range(0, MCNominal.GetNbinsX()):
	        MCTemplates[i].SetBinContent( j+1, MCTemplatesO[i].GetBinContent(j+1) + MCNominal.GetBinContent(j+1)  )
...
Variable    = "mT"
MCToys      = ReadToysMC(Data_B, channel, Variable, indice[kindice])
MCTemplates = ReadMCTemplate(MC, channel, Variable, indice[kindice])
MCNominal   = ReadMCFiles(Data,  channel, Variable, indice[kindice])
